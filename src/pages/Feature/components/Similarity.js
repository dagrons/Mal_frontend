...
export default () => {
  ...
  return (
    <div>
      ...
      <Row>
        <Paragraph>
          恶意代码起源分析基于Doc2Vec与知识图谱，将第一阶段的恶意代码样本经过基因特征提取模型得到的opcode切片为若干语义序列，划分为若干恶意语义句。经过Doc2Vec模型将恶意代码编码为低纬度高层级的抽象语义信息，组合所有的语义向量，在向量空间中寻找分布程度最为接近的恶意代码样本。根据同源分析的结果，系统可以找出关联程度最高的样本，并根据样本之间的时间节点做出攻击推定。
          除此，通过提取出的恶意代码关联特征建立知识图谱，有效挖掘潜在的APT组织关联关系，并作出更为精确的攻击推断
        </Paragraph>
      </Row>
    </div>
  );
